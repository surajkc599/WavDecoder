 ...
 */
function WavDecoder(opts) {
  this.readerMeta = false;
  this.opts = opts || {};
}

WavDecoder.prototype.decodeChunk = function(arrayBuffer) {
  return new Promise(resolve => {
    resolve(this.decodeChunkSync(arrayBuffer));
  });
}

WavDecoder.prototype.decodeChunkSync = function(arrayBuffer) {
  let reader = new WavReader(new DataView(arrayBuffer));
...
WavDecoder.prototype._init = function(reader) {
...
WavDecoder.prototype._decodeMetaInfo = function (reader, chunkSize) {
  const formats = {
    0x0001: "lpcm",
    0x0003: "lpcm"
  };
...
WavDecoder.prototype._decodeData = function(reader) {
 ...
 */
function WavReader(dataView) {
  this.view = dataView;
  this.pos = 0;
}

WavReader.prototype.remain = function() {
  return this.view.byteLength - this.pos;
},

WavReader.prototype.skip = function(n) {
  this.pos += n;
},

WavReader.prototype.uint8 = function() {
  const data = this.view.getUint8(this.pos, true);
  this.pos += 1;
  return data;
},

WavReader.prototype.int16 = function() {
  const data = this.view.getInt16(this.pos, true);
  this.pos += 2;
  return data;
},

WavReader.prototype.uint16 = function() {
  const data = this.view.getUint16(this.pos, true);
  this.pos += 2;
  return data;
},

WavReader.prototype.uint32 = function() {
  const data = this.view.getUint32(this.pos, true);
  this.pos += 4;
  return data;
},

WavReader.prototype.string = function(n) {
  let data = "";
  for (let i = 0; i < n; i++) {
...
WavReader.prototype.pcm8 = function() {
  const data = this.view.getUint8(this.pos) - 128;
  this.pos += 1;
  return data < 0 ? data / 128 : data / 127;
},

WavReader.prototype.pcm8s = function() {
  const data = this.view.getUint8(this.pos) - 127.5;
  this.pos += 1;
  return data / 127.5;
},

WavReader.prototype.pcm16 = function() {
  const data = this.view.getInt16(this.pos, true);
  this.pos += 2;
  return data < 0 ? data / 32768 : data / 32767;
},

WavReader.prototype.pcm16s = function() {
  const data = this.view.getInt16(this.pos, true);
  this.pos += 2;
  return data / 32768;
},

WavReader.prototype.pcm24 = function() {
...
WavReader.prototype.pcm24s = function() {
  let x0 = this.view.getUint8(this.pos + 0);
  let x1 = this.view.getUint8(this.pos + 1);
  let x2 = this.view.getUint8(this.pos + 2);
  let xx = (x0 + (x1 << 8) + (x2 << 16));
...
WavReader.prototype.pcm32 = function() {
  const data = this.view.getInt32(this.pos, true);
  this.pos += 4;
  return data < 0 ? data / 2147483648 : data / 2147483647;
},

WavReader.prototype.pcm32s = function() {
  const data = this.view.getInt32(this.pos, true);
  this.pos += 4;
  return data / 2147483648;
},

WavReader.prototype.pcm32f = function() {
  const data = this.view.getFloat32(this.pos, true);
  this.pos += 4;
  return data;
},

WavReader.prototype.pcm64f = function() {
